...
const ConfirmSeedPhrase = ({ navigation, route }) => {
  ...
  const handleNext = () => {
    ...
    if (inputPhrase[randomPosition] === seedPhrase[randomPosition]) {
      ...
      if (activeStep === 3) {
        navigation.navigate('SuccessSeedPhrase');
      } else {
  ...
  return (
    <View style={styles.container}>
      <View style={styles.rectanglesContainer}>
        <View style={[styles.rectangle, activeStep >= 1 && styles.activeRectangle]}></View>
        <View style={[styles.rectangle, activeStep >= 2 && styles.activeRectangle]}></View>
        <View style={[styles.rectangle, activeStep === 3 && styles.activeRectangle]}></View>
      </View>
      ...
      <View style={styles.rectanglesContainer}>
        {inputPhrase.map((word, index) => (
          <View
            ...
            style={[
              styles.rectangle,
              { backgroundColor: randomPosition === index ? '#FEBF32' : '#222531' }
            ]}
          />
        ))}
      </View>
      <TouchableOpacity
        style={[styles.button, activeStep <= 3 && inputPhrase[randomPosition] === seedPhrase[randomPosition] ? styles.activeButton : styles.disabledButton]}
        onPress={handleNext}
        disabled={!(activeStep <= 3 && inputPhrase[randomPosition] === seedPhrase[randomPosition])}
      >
        <Text style={styles.buttonText}>{activeStep === 3 ? 'Next' : 'Continue'}</Text>
      </TouchableOpacity>
    </View>
  );
};
